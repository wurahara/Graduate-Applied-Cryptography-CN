...
\begin{figure}
  \centering
  \subfigure[加密]{\includegraphics[width=0.6\linewidth]{figures/chapter4/fig4-a.png}}
  \subfigure[解密]{\includegraphics[width=0.6\linewidth]{figures/chapter4/fig4-b.png}}
  \caption{ECB模式的加密与解密}
  \label{fig:4-4}
\end{figure}
...
\begin{quote}
特别地，对于每个就 $\mathcal{E}'$ 进行攻击游戏 \ref{game:2-1} 的 SS 对手 $\mathcal{A}$，都存在一个就 $\mathcal{E}$ 进行攻击游戏 \ref{game:4-1} 的 BC 对手 $\mathcal{B}$，其中 $\mathcal{B}$ 是围绕 $\mathcal{A}$ 的一个基本包装器，满足：
\end{quote}
